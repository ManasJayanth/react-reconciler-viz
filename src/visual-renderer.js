...
import * as DOMSideEffects from './dom-side-effects';

let dsHandle = DOMSideEffects.init();
const hostConfig = {
  ...
  createInstance(
    ...
    //   hostContext,
    // );
    dsHandle = DOMSideEffects.queue(dsHandle, DOMSideEffects.newEffect('createElement', { type }))
  ...
  finalizeInitialChildren(
    ...
    Object.keys(otherProps).forEach(attr => {
      if (attr === 'className') {
        ...
        throw new Error(`TODO: We haven't handled other properties/attributes. Got ${attr}`)
      }
    })
...
export function render(elements, containerNode, callback) {
...
export function retrieve() {
  return DOMSideEffects.retrieveQueue(dsHandle);
}
